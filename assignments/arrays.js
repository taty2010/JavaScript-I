...
let carYears = inventory.filter(function(hero) {
	return hero.car_year < 2000;
});


for (i = 0; i < carYears.length; i++){
	 let oldCars = carYears[i].car_year;
  console.log(oldCars);
}
...
let list = inventory.filter(function(car) {
	return (car.car_make == "BMW" || car.car_make == "Audi"); 
});

let bMWAndAudi = JSON.stringify(list);
